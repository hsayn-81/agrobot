...
function About() {
  ...
  return (
    ...
    >
      {/* Background glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[400px] md:w-[600px] h-[400px] md:h-[600px] bg-green-500/5 rounded-full blur-3xl" />
      ...
      <div className="relative max-w-6xl mx-auto">
        ...
        >
          <h3 className="text-xl md:text-3xl font-bold text-center mb-8 md:mb-12">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
            {problems.map((problem, idx) => (
              <motion.div
                ...
                whileHover={{ y: -8 }}
                className="group p-6 md:p-8 bg-zinc-900/50 border border-white/5 rounded-2xl backdrop-blur-sm hover:border-green-500/50 transition-all duration-300"
              >
                <div className="text-green-400 mb-3 md:mb-4 group-hover:scale-110 transition-transform duration-300">
                  {problem.icon}
                </div>
                <h4 className="text-lg md:text-xl font-bold mb-2 md:mb-3">{problem.title}</h4>
                <p className="text-gray-400 text-sm md:text-base leading-relaxed">{problem.desc}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          className="relative p-6 md:p-10 lg:p-16 rounded-2xl md:rounded-3xl bg-gradient-to-br from-green-500/10 via-zinc-900/50 to-cyan-500/10 border border-green-500/20 text-center overflow-hidden"
        >
